...
function Uploadn1({ moduleNumber }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        'https://3f2ssd7loqowjtj7hnzhni7trq0blutk.lambda-url.us-east-1.on.aws/notestotext_modwise',
        formData
      );
    ...
    <div className="flex flex-col items-center justify-center w-screen text-center h-screen bg-gradient-to-tr from-violet-700 via-green-600 to-green-400">
      {!fadeOut && (
        ...
        >
          ...
          <Link to="/uploadp1" className="bg-green-500 text-white py-2 ml-4 px-6 mt-4 rounded-lg">
            Next
          </Link>
        </motion.div>
      )}
